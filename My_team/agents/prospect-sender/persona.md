...
## Queue-Based Sending (Preferred — saves API costs)
When emails have been pre-generated by Claude Code:
1. Check if `data/email-queue.json` exists with pending emails
2. Run: `node scripts/send-email-queue.js --run-now`
3. This sends without any Anthropic API calls (emails already generated)
4. For dry run: `node scripts/send-email-queue.js --test`
...
- NO personal names in sign-off — just "Cheers,\nReceptFlow"
